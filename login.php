...
<body>
    ...
    <main>
        ...
        <form action= "<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>" method ="post" class = "login-form">
            ...
            <h2 class="login-form-title"><a class="login-form-anchor" href="signup.php">Click here to sign up !<a></h2>
            <div class="login-label-input-container">
                <label class="login-form-sub-title">Username:</label>
                <input class="input-username-password" type="text" name="username" autocomplete="off">
                <label class="login-form-sub-title">Password:</label>
                <input class="input-username-password" type="password" name="password" autocomplete="off">
            </div>
            <input class="input-login-button" type="submit" name="submit" value="Login">
            <div class="login-form-php-container">
                <?php
                        if($_SERVER["REQUEST_METHOD"] == "POST"){
                            $username = filter_input(INPUT_POST, "username", FILTER_SANITIZE_SPECIAL_CHARS);
                            $password = filter_input(INPUT_POST, "password", FILTER_SANITIZE_SPECIAL_CHARS);
                            if(empty($username)){
                            echo "<p class = 'login-error-message'>Please enter your username.</p><br>";
                            }
                            else{
                            if(empty($password)){
                                echo "<p class = 'login-error-message'>Please enter your password.<p><br>";
                            }
                            else{
                                $sql_retrieve = "SELECT * FROM necktapers_users WHERE username  = '$username' ";
                                ...
                                if(password_verify($password, $row["password"]) and $username === "necktapers_admin"){
                                    header("Location: admin.php");
                                    ob_end_flush();
                                    $_SESSION["username"] = $_POST["username"];
                                    $_SESSION["password"] = $_POST["password"];
                                    // or instead of header() we can use ==> echo '<script type= "text/javascript">window.location = "appointment.php"</script>';
                                    ...
                                    if(password_verify($password, $row["password"]) and $username !== "necktapers_admin"){
                                        ...
                                        echo "<p class = 'login-error-message'>Incorrect Password.</p><br>";
                                    }
                                }
                                }
                                else{
                                echo "<p class='login-error-message'>No user found.<br><a class='sign-up-message' href='signup.php'>Click here to Sign up !<br></a></p>";
                                }
                            }
                            }
                        }    
