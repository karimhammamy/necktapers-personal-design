...
<head>
    ...
    <!-- Inline CSS for slider area here -->
    <style>.slider-height2{height:260px !important;}@media only screen and (min-width:992px) and (max-width:1919px) {.slider-height2{height:350px !important;}} @media only screen and (min-width:1920px){.slider-height2{height:550px !important;}.footer-area{padding-top: 100px;;padding-bottom: 100px;}}</style>
...
<body>
    ...
    <main>
        ...
        <!-- Hero End -->
        <div class="hide-include-database">
              <?php
                  include("database.php");
              ?>
        </div>
        <form  action= "<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>" method ="post" class="sign-up-form" >
          <h2 class="sign-up-form-title">Create your account,</h2>
          <h2 class="sign-up-form-title"><a class="sign-up-form-anchor" href="login.php">Click here to login !<a></h2>
          <div class="sign-up-label-input-container">
            <label class="sign-up-form-sub-title">username:</label>
            <input class="input-username-password" type="text" name="username" autocomplete="off">
            <label class="sign-up-form-sub-title">password:</label>
            <input class="input-username-password" type="password" name="password" autocomplete="off">
          </div>
          <input class="input-sign-up-button" type="submit" name="submit" value="Sign Up"><br>
          <div class="sign-up-form-php-container">
            <?php
              if($_SERVER["REQUEST_METHOD"] == "POST"){
                $username = filter_input(INPUT_POST, "username", FILTER_SANITIZE_SPECIAL_CHARS);
                $password = filter_input(INPUT_POST, "password", FILTER_SANITIZE_SPECIAL_CHARS);
                if(empty($username)){
                  echo "<p class = 'sign-up-error-message'>Please enter a username.</p><br>";
                }
                else{
                  if(preg_match('/\s/',$username)){
                    echo "<p class = 'sign-up-error-message'>Invalid username.</p>";
                    echo "<p class = 'sign-up-message'>Please remove any spaces.</p>";
                  }
                  else{
                    if(empty($password)){
                      echo "<p class = 'sign-up-error-message'>Please enter a password.</p><br>";
                    }
                    else{
                      if(preg_match('/\s/',$password)){
                        echo "<p class = 'sign-up-error-message'>Invalid password.</p>";
                        echo "<p class = 'sign-up-message'>Please remove any spaces.</p>";
                      }
                      else{
                        $hash = password_hash($password, PASSWORD_DEFAULT);
                        $sql = "INSERT INTO necktapers_users (username, password) VALUES('$username', '$hash')";
                        try{
                          mysqli_query($connection, $sql);
                          echo "<p class = 'sign-up-success-message'>You are now registered !</p><br>";
                          echo "<p class = 'sign-up-success-message'>Login to your account to make your appointment.</p><br>";
                        }
                        catch(mysqli_sql_exception){
                          echo "<p class = 'sign-up-error-message'>This username is taken.</p>";
                          echo "<p class = 'sign-up-message'>If this is your account,</p>";
                          echo "<a class = 'sign-up-message' href='login.php'>Click here to login !</a>";
                          //echo "<p class = 'sign-up-error-message'>MySQL Error: </p>" . mysqli_error($connection);
                        }
                      }
                    }
    ...
    <!-- slider area animation js -->	
    <script src="./assets/js/slider-area-animation.js"></script>

    <!-- Service Icons Color Change -->	
    <script src="./assets/js/service-icons.js"></script>

     <!-- Unify Services divs heights -->	
     <script src="./assets/js/unify.services.divs.heights.js"></script>

     <script src="./assets/js/scroll-to-error-message.js"></script>

     <!-- gallery area js -->	
     <script src="./assets/js/insta-container.js"></script>

     <!-- Slider area video -->	
     <script src="./assets/js/slider-area-video.js"></script>
